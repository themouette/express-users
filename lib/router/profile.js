...
module.exports = function (sanitizer, store, passwordChecker, render) {
    ...
    function encryptUserPassword(user, next) {
        ...
        config.passwordChecker.hashPassword(user, function (err, newUser) {
            if (err) {
                // forward error
    ...
    return {
        ...
        updatePassword: [
            function sanitizePasswordUpdate(req, res, next) {
                config.sanitizer.sanitizePasswordUpdate(req.body, function (err, updatedFields) {
                    if (err) {
            ...
            function updateUser(req, res, next) {
                config.store.updateUserPassword(req.user, req.sanitizedUser, function (err, user) {
                    if (err) {
